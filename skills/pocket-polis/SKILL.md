...
3. Prefer the deployment's Streamable HTTP MCP endpoint at `/mcp` when the host supports remote MCP; otherwise use the REST API. For a new conversation: draft 5–15 seed statements with the user (single-idea, votable sentences, ≤280 chars), pick moderation mode (`autoApprove`) and data openness (`openData`), then call `create_conversation` (or `POST /api/conversations`). Hand the admin link (`/a/:id#token=…`) to the user privately — the token is shown once and cannot be recovered.
