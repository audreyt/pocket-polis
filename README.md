...
**Free-tier neuron contract (hard ceiling):** `@cf/google/gemma-4-26b-a4b-it` is billed `neurons = input_tokens × 9091 / 1e6 + output_tokens × 27273 / 1e6` ([Workers AI pricing](https://developers.cloudflare.com/workers-ai/platform/pricing/)). Input tokens are a conservative **upper bound** `utf8_bytes(system)+utf8_bytes(user)+256` chat-template overhead — not JS `string.length`, and not an exact tokenizer count. Output uses each call's enforced `max_tokens` (discovery 2048, categorize batch 1024, synthesis 4096). Before every `ai.run` the worker synchronously reserves that call's worst-case neurons on a per-generation ledger capped at **9,000** (headroom below 10,000 free neurons/day). A synthesis-phase hold is taken first so optional categorize retries cannot starve the final call. Prompts are UTF-8-byte capped (discovery 240,000, categorize batch 32,000, synthesis 48,000) and every statement ID is preserved; consensus and tension evidence are ranked and capped at 24 each. If admission or a phase cannot fit, the result is a cacheable deterministic statistical report (`generationMode: "deterministic"`, `model: "deterministic"`) — never labeled as Gemma. **Queues are durability/latency isolation, not neuron savings.** One `<64KB` message with `max_retries: 1` costs at most **4 Queue operations** (1 write + 2 reads + 1 delete; success is 3). That meter is independent of neurons.
## Quick start
